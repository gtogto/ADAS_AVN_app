...
public class newAccSlidingCollection {
    ...
    public newGestureRecognition GR2 = new newGestureRecognition();

    private int [][] average_buffer = { {X_INIT, X_INIT, X_INIT, X_INIT}, {Y_INIT, Y_INIT, Y_INIT, Y_INIT}, {Z_INIT, Z_INIT, Z_INIT, Z_INIT}};
    private int [][] sensors = new int [GR2.AXIS_NUM][GR2.SAMPLE_NUM];
    ...
    public int SlidingCollectionInterface (int [] recv)	{
       ...
       // Log.d (TAG, recv [0] + " " + recv [1] + " " + recv [2]);
        ...
        sensors [GR2.ACC_X][counter] = recv [GR2.ACC_X];
        sensors [GR2.ACC_Y][counter] = recv [GR2.ACC_Y];
        sensors [GR2.ACC_Z][counter ++] = recv [GR2.ACC_Z];



        if (counter == GR2.SAMPLE_NUM)	{		//collect sample to SAMPLE_NUM
            counter = 0;

            if (processFlag)	{	// hit && collect
                processFlag = false;

                result_ = GR2.gestureRecogInterface (sensors, getBandStatus());
    ...
    private void setBandStatus ()  {
        ...
        if ((Y_CALM < LEAM_TH_SMALL_Y) && (Z_CALM < LEAM_TH_SMALL_Z))    {
    ...
    private boolean ThresholdCheck	(int [] recv)	{
        ...
        if (((recv [GR2.ACC_X] > X_CALM + MOVE_THRES) || (recv [GR2.ACC_X] <  X_CALM - MOVE_THRES)) && ( Math.abs(Math.abs(recv [GR2.ACC_X]) - Math.abs(bax)) > GAP_THRES))	{
          //  Log.d (TAG, "--------------------- X -------------- "+counter +" "+ Math.abs(Math.abs(recv [GR.ACC_X]) - Math.abs(bax)) + " " + bax);
            reutrn_ = true;
        }
        else if ((( recv [GR2.ACC_Y] >  Y_CALM + MOVE_THRES) || (recv [GR2.ACC_Y] < Y_CALM - MOVE_THRES)) && (Math.abs( Math.abs(recv [GR2.ACC_Y]) - Math.abs(bay)) > GAP_THRES))	{
          //  Log.d (TAG, "--------------------- Y -------------- "+counter+" "+ Math.abs(Math.abs(recv [GR.ACC_Y]) - Math.abs(bay))+ " " + bay);
            reutrn_ = true;
        }
        else if (((recv [GR2.ACC_Z] >  Z_CALM + MOVE_THRES) || (recv [GR2.ACC_Z] < Z_CALM - MOVE_THRES)) && (Math.abs( Math.abs(recv [GR2.ACC_Z]) - Math.abs(baz)) > GAP_THRES))	{
          //  Log.d (TAG, "--------------------- Z -------------- "+counter);
            reutrn_ = true;
        }
    ...
    private boolean SaveSlidedElement ()	{
        ...
        if (counter > BACKUP_SAMPLE)	{ 	//bigger than 5th sample
            for (nLoop = 0; nLoop < GR2.AXIS_NUM; nLoop ++)	{
            ...
            for (nLoop = 0; nLoop < GR2.AXIS_NUM; nLoop ++)	{
                ...
                for (nLoop2 = 0; nLoop2 < tempInt; nLoop2 ++)	{
                    sensors [nLoop][tempInt - nLoop2 - 1] = sensors [nLoop][GR2.SAMPLE_NUM -1 - nLoop2];
    ...
    private void AverageCalculator (int [] recv) {
        ...
        for (int nLoop = GR2.ACC_X; nLoop <  ITER_VALUE; nLoop ++)  {
            average_buffer [GR2.ACC_X][nLoop] = average_buffer [GR2.ACC_X][nLoop +1];
            average_buffer [GR2.ACC_Y][nLoop] = average_buffer [GR2.ACC_Y][nLoop +1];
            average_buffer [GR2.ACC_Z][nLoop] = average_buffer [GR2.ACC_Z][nLoop +1];
        }
        average_buffer [GR2.ACC_X][ITER_VALUE] = recv [GR2.ACC_X];
        average_buffer [GR2.ACC_Y][ITER_VALUE] = recv [GR2.ACC_Y];
        average_buffer [GR2.ACC_Z][ITER_VALUE] = recv [GR2.ACC_Z];


        X_CALM = Y_CALM = Z_CALM = 0;
        for (int nLoop = GR2.ACC_X; nLoop <=  ITER_VALUE; nLoop ++)  {
            X_CALM += average_buffer [GR2.ACC_X][nLoop];
            Y_CALM += average_buffer [GR2.ACC_Y][nLoop];
            Z_CALM += average_buffer [GR2.ACC_Z][nLoop];;
        }
    ...
    private static final String TAG = "newASC";

    private static int X_INIT = 0;
    private static int Y_INIT = 0;
    private static int Z_INIT = 16000;

    private static int MOVE_THRES = 5000;
    private static int GAP_THRES = 7000;
    ...
    // private static int LEAM_TH_X = 0; //not used in new band
    private static int LEAM_TH_SMALL_Y = -13000;
    private static int LEAM_TH_SMALL_Z = 5000;
